...
| **Нет инверсных токенов** | На тёмном фоне (`bg-contrast`) полноценного набора text/icon/border нет — только `text-contrast` и `icon-contrast` (белый). Тёмные блоки делать не получится. |
...
| **`bg-contrast` сирота** | Токен есть, но раз тёмные блоки не используются, на практике не применяется. Удалим, если так и не понадобится. |
